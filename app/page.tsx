...
const SleepAnalyzer = () => {
  ...
  const changeMonth = (direction: number) => {
    ...
    // Only allow navigation if it's not to a future month
    if (!wouldBeFuture) {
  ...
  const TimelineSlide = () => {
    return (
      ...
      >
      {/* Unified Component Layout with integrated header */}
      <div className="flex-1 bg-gray-800/50 backdrop-blur-md rounded-2xl border border-gray-700/30 shadow-2xl overflow-hidden flex flex-col">
        {/* Desktop Header with Sleep Schedule and Average */}
        <div className="flex items-center justify-between p-4 border-b border-gray-700/30">
          ...
          <button
            ...
            className="flex items-center space-x-2 px-4 py-2 bg-gradient-to-r from-purple-500 to-pink-500 text-white rounded-xl shadow-lg hover:from-purple-600 hover:to-pink-600 transition-all duration-300 transform hover:scale-105 cursor-pointer"
        ...
        <div className="flex-1 overflow-hidden flex">
          ...
          <div className="w-80 flex-shrink-0 p-6 pb-4 bg-gray-800/40 rounded-xl shadow-lg h-full max-h-full overflow-y-auto scrollbar-hide flex flex-col hide-scrollbar">
            <div className="flex-1 min-h-0 flex flex-col gap-y-8.5">
              ...
              {/* Monthly Stats anchored at the bottom, compact */}
              <div className="pb-4">
            ...
            <div className="flex-1 bg-gray-700/20 rounded-xl p-6 min-h-0 relative" style={{ zIndex: 1 }}>
              {isFutureMonth ? (
                ...
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={generateMonthlySleepData} margin={{ top: 20, right: 20, left: 10, bottom: 20 }}>
  ...
  const RadarSlide = () => {
    return (
      <motion.div
        ...
        className="w-full h-full flex flex-col overflow-hidden"
      >
      <div className="flex-1 bg-gray-800/50 backdrop-blur-md rounded-2xl p-6 border border-gray-700/30 shadow-2xl overflow-hidden flex flex-col">
        {/* Metrics summary inside the card, at the top */}
        ...
        <div className="grid grid-cols-1 xl:grid-cols-2 gap-4 flex-1 min-h-0">
          ...
          <div className="flex flex-col min-h-0">
            ...
            <div className="space-y-3 overflow-y-auto flex-1 scrollbar-hide" style={{ maxHeight: 'calc(100vh - 320px)' }}>
              ...
              {/* Desktop Hover Tooltip for Sleep Phase Details */}
                ...
                <div
                  ...
                  style={{
                    left: (() => {
                      ...
                      // If tooltip would go beyond left container edge
                      if (leftPos < containerLeft + 20) {
                        leftPos = containerLeft + 20; 
                      } 
                      // If tooltip would go beyond right container edge
                    ...
                    top: (() => {
                      ...
                      if (topPos < containerTop) {
                        ...
                        // If it would also go below container when positioned below
                        if (topPos + tooltipHeight > containerBottom) {
                          topPos = containerBottom - tooltipHeight - 20;
                        }
                      }
                      // If tooltip would go below container bottom
                      else if (topPos + tooltipHeight > containerBottom) {
                        topPos = containerBottom - tooltipHeight - 20;
                      }
                      return Math.max(topPos, containerTop + 20); 
                    })(),
                    width: '280px',
                    maxWidth: '280px'
                  }}
    ...
    return (
      ...
      <div className="flex-1 bg-gray-800/50 backdrop-blur-md rounded-2xl p-6 border border-gray-700/30 shadow-2xl overflow-hidden flex flex-col">
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
          {(() => {
            if (!Object.keys(manualSleepData).length) {
              return [
                {
                  icon: FaCoffee,
                  label: 'Caffeine',
            ...
            const envData = Object.values(manualSleepData)
              .filter((data: any) => data.environmental)
              .map((data: any) => data.environmental);
            if (!envData.length) {
              return [
                {
                  icon: FaCoffee,
            ...
            const avgCaffeine = envData.reduce((sum, env) => sum + env.caffeine, 0) / envData.length;
            const avgExercise = envData.reduce((sum, env) => sum + env.exercise, 0) / envData.length;
            const avgTemperature = envData.reduce((sum, env) => sum + env.temperature, 0) / envData.length;
            return [
              {
                icon: FaCoffee,
                label: 'Caffeine',
        ...
        <div className="flex-1 bg-gray-700/20 rounded-xl p-6 min-h-0 flex flex-col">
          ...
          <div className="flex-1 min-h-0">
            {correlationData.length === 0 ? (
              ...
              <ResponsiveContainer width="100%" height="100%">
                <ScatterChart data={correlationData} margin={{ top: 10, right: 10, left: 10, bottom: 10 }}>
  ...
  const CustomTimePicker = ({ value, onChange, placeholder }: { value: string; onChange: (time: string) => void; placeholder: string }) => {
    ...
    return (
      <div className="relative time-picker-container">
        ...
        {showTimePicker && (
          <div className="fixed inset-x-2 top-1/2 transform -translate-y-1/2 sm:absolute sm:top-full sm:left-1/2 sm:transform sm:-translate-x-1/2 sm:translate-y-0 sm:inset-auto mt-0 sm:mt-2 bg-gray-800/95 backdrop-blur-md rounded-xl border border-gray-600/50 shadow-2xl p-3 sm:p-4 z-[9999] w-auto sm:w-72 max-w-xs sm:max-w-none mx-auto">
            {/* Time Display */}
            ...
            <div className="grid grid-cols-3 gap-1 sm:gap-3 mb-2 sm:mb-4">
              ...
              <div className="text-center">
                <div className="text-gray-300 text-xs font-semibold mb-1 sm:mb-2">Hour</div>
                <div className="h-20 sm:h-32 overflow-y-auto scrollbar-hide bg-gray-700/30 rounded-lg p-1 sm:p-2">
                  {Array.from({ length: 12 }, (_, i) => {
                    const hour = i + 1;
              ...
              {/* Minutes */}
              <div className="text-center">
                <div className="text-gray-300 text-xs font-semibold mb-1 sm:mb-2">Minute</div>
                <div className="h-20 sm:h-32 overflow-y-auto scrollbar-hide bg-gray-700/30 rounded-lg p-1 sm:p-2">
                  {Array.from({ length: 12 }, (_, i) => {
                    const minute = i * 5;
              ...
              {/* AM/PM */}
              <div className="text-center">
                <div className="text-gray-300 text-xs font-semibold mb-1 sm:mb-2">Period</div>
                <div className="h-20 sm:h-32 bg-gray-700/30 rounded-lg p-1 sm:p-2 flex flex-col justify-center space-y-1 sm:space-y-2">
                  {['AM', 'PM'].map((period) => {
            ...
            <div className="flex justify-between pt-1 sm:pt-2 border-t border-gray-700/50">
              <button
                ...
                className="text-gray-300 hover:text-white text-xs sm:text-sm transition-colors cursor-pointer py-1 px-2 hover:bg-white/10 hover:backdrop-blur-sm rounded-lg"
              >
                Cancel
              </button>
              <button
                ...
                className="text-purple-400 hover:text-purple-300 text-xs sm:text-sm transition-colors cursor-pointer py-1 px-2 hover:bg-purple-500/10 rounded-lg"
              >
                Select
              </button>
            </div>
      ...
      <div className="relative date-picker-container">
        ...
        >
          ...
          <FaCalendarAlt className="text-purple-400 text-sm" />
                 ...
                 {showDatePicker && (
           <div className="fixed inset-x-2 top-1/2 transform -translate-y-1/2 sm:absolute sm:top-full sm:left-1/2 sm:transform sm:-translate-x-1/2 sm:translate-y-0 sm:inset-auto mt-0 sm:mt-2 bg-gray-800/95 backdrop-blur-md rounded-xl border border-gray-600/50 shadow-2xl p-3 sm:p-4 z-[9999] w-auto sm:w-80 max-w-sm sm:max-w-none mx-auto">
                         {/* Year and Month Navigation */}
             <div className="flex items-center justify-center mb-4 sm:mb-6">
               <button
                 type="button"
                 onClick={() => setSelectedYear(selectedYear - 1)}
                 className="text-gray-300 hover:text-white transition-colors p-2 hover:bg-white/10 hover:backdrop-blur-sm rounded-lg cursor-pointer touch-target"
               >
                 <FaChevronLeft className="text-sm" />
               </button>
               <div className="flex flex-col items-center mx-4 sm:mx-8">
                 <div className="text-white text-lg sm:text-xl font-semibold mb-1 sm:mb-2">{selectedYear}</div>
                 <div className="relative month-selector-container">
                   <button
                     type="button"
                     onClick={() => setShowMonthSelector(!showMonthSelector)}
                     className="bg-gray-700/50 text-white text-xs sm:text-sm rounded-lg px-2 sm:px-3 py-1 sm:py-2 border border-gray-600 focus:outline-none focus:ring-2 focus:ring-purple-500 cursor-pointer min-w-24 sm:min-w-32 text-center hover:bg-white/10 hover:backdrop-blur-sm transition-all duration-200 flex items-center justify-center space-x-1"
                   >
             ...
             <div className="grid grid-cols-7 gap-1 sm:gap-2 mb-2 sm:mb-3">
               {['Su', 'Mo', 'Tu', 'We', 'Th', 'Fr', 'Sa'].map((day) => (
                 <div key={day} className="text-gray-300 text-xs text-center py-1 sm:py-2 font-semibold">
                   {day}
                 </div>
               ))}
             </div>
             ...
             <div className="grid grid-cols-7 gap-1 sm:gap-2">
               ...
               {Array.from({ length: firstDayOfMonth }, (_, i) => (
                 <div key={`empty-${i}`} className="h-8 w-8 sm:h-10 sm:w-10"></div>
               ))}
               {/* Days of the month */}
               {Array.from({ length: daysInMonth }, (_, i) => {
                 ...
                 return (
                   <button
                     ...
                     className={`h-8 w-8 sm:h-10 sm:w-10 rounded-lg text-xs sm:text-sm font-medium transition-all duration-200 cursor-pointer flex items-center justify-center touch-target ${
                       isSelected
                         ? 'bg-gradient-to-r from-purple-500 to-pink-500 text-white shadow-lg'
                         : isToday
                   ...
                   >
                     {day}
                         ...
                         {/* Action buttons */}
             <div className="flex justify-between mt-3 sm:mt-4 pt-3 border-t border-gray-700/50">
               <button
                 type="button"
                 onClick={() => setShowDatePicker(false)}
                 className="text-gray-300 hover:text-white text-xs sm:text-sm transition-colors cursor-pointer py-2 px-3 hover:bg-white/10 hover:backdrop-blur-sm rounded-lg touch-target"
               >
                 Cancel
               </button>
               <button
                 ...
                 className="text-purple-400 hover:text-purple-300 text-xs sm:text-sm transition-colors cursor-pointer py-2 px-3 hover:bg-purple-500/10 rounded-lg touch-target"
               >
                 Today
               </button>
             </div>
  ...
  const AddDataModal = () => {
    ...
    const handleSubmit = (e: React.FormEvent) => {
      ...
      console.log('Added sleep data:', sleepData);
    ...
    return (
      <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-[9999] flex items-center justify-center p-4">
        <motion.div
          ...
          className="bg-gray-800/95 backdrop-blur-md rounded-2xl p-6 max-w-2xl w-full border border-gray-700/50 shadow-2xl max-h-[90vh] overflow-y-auto"
        >
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="space-y-4">
              <h4 className="text-lg font-semibold text-white flex items-center space-x-2">
                ...
                <span className="text-sm text-gray-300 font-normal">Total: {totalSleep.toFixed(1)}h</span>
              </h4>
            ...
            <div className="space-y-4">
              ...
              <div className="bg-gray-700/30 rounded-lg p-4">
                <div className="flex items-center justify-between mb-3">
                  <span className="text-gray-300 text-sm">Calculated Quality Score:</span>
              ...
              <div className="grid grid-cols-3 gap-4">
                <div>
                  <label className="block text-sm font-medium text-gray-300 mb-2 flex items-center space-x-1">
                    ...
                    <span>Caffeine: {caffeine} cups</span>
                  </label>
                  <input
                    type="range"
                    min="0"
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-300 mb-2 flex items-center space-x-1">
                    ...
                    <span>Exercise: {exercise} min</span>
                  </label>
                  <input
                    type="range"
                    min="0"
            ...
            <div className="flex space-x-3 pt-4 border-t border-gray-700/50">
              <button
                type="button"
                onClick={() => setShowAddDataModal(false)}
                className="flex-1 px-4 py-2 bg-gray-700 text-gray-300 rounded-lg hover:bg-white/10 hover:backdrop-blur-sm transition-all duration-300 cursor-pointer"
              >
                Cancel
              </button>
              <button
                type="submit"
                className="flex-1 px-4 py-2 bg-gradient-to-r from-purple-500 to-pink-500 text-white rounded-lg hover:from-purple-600 hover:to-pink-600 transition-all duration-300 font-medium cursor-pointer"
              >
                Add Sleep Data
              </button>
            </div>
        ...
        <div className="max-w-md mx-auto" style={{ scrollbarWidth: 'none', msOverflowStyle: 'none' }}>
          ...
          >
            <div className="flex items-center justify-between">
              ...
              {/* Add Data Button for mobile - round and aligned */}
              <button
                onClick={() => setShowAddDataModal(true)}
                className="flex items-center justify-center w-12 h-12 bg-gradient-to-r from-purple-500 to-pink-500 text-white rounded-full shadow-lg hover:from-purple-600 hover:to-pink-600 transition-all duration-300 cursor-pointer transform hover:scale-105"
                title="Add Sleep Data"
              >
          ...
          <div className="space-y-6">
            {/* Timeline Slide - mobile responsive */}
            <motion.div
              initial={{ opacity: 0, y: 50 }}
              animate={{ opacity: 1, y: 0 }}
              transition={{ delay: 0 }}
              className="bg-gray-800/90 backdrop-blur-md rounded-3xl p-6 min-h-96 border border-gray-700/20 flex flex-col shadow-2xl"
            >
              {/* Card Title at the top */}
              <div className="w-full mb-6">
                <h2 className="text-xl sm:text-2xl font-bold text-white text-left mb-1 flex items-center whitespace-nowrap">
                  ...
                  <div className="w-full bg-gray-700/20 rounded-2xl p-4 overflow-hidden">
                    <div className="overflow-x-auto">
                      <div className="min-w-[380px] h-56">
                        <ResponsiveContainer width="100%" height="100%">
                          <BarChart data={generateMonthlySleepData} margin={{ top: 10, right: 10, left: 20, bottom: 10 }}>
                            <CartesianGrid strokeDasharray="3 3" stroke="#4B5563" />
                            <XAxis
                              dataKey="day"
                              stroke="#9CA3AF"
            ...
            >
              <div className="w-full mb-6">
                ...
                <p className="text-gray-300 text-sm">Detailed breakdown of your sleep metrics</p>
              </div>
              {/* Average metric cards for mobile */}
              ...
              {/* Sleep Phases summary - responsive */}
            ...
            {/* Correlation Slide - mobile responsive */}
            <motion.div
              initial={{ opacity: 0, y: 50 }}
              animate={{ opacity: 1, y: 0 }}
            ...
            >
              ...
              {/* Cards for factors - compact mobile design */}
      ...
      <div className="h-full flex flex-col max-w-7xl mx-auto">
        ...
        <div className="flex-1 relative px-2 pb-4 min-h-0">
          ...
          >
            ...
            {/* Navigation buttons: only visible on hover (desktop), hidden on mobile */}
            <div className="absolute top-1/2 left-2 -translate-y-1/2 flex items-center z-10 opacity-0 group-hover:opacity-100 transition-opacity duration-300 pointer-events-none group-hover:pointer-events-auto hidden md:flex">
              <button
                onClick={prevSlide}
                className="p-0 m-0 bg-transparent border-none outline-none focus:outline-none cursor-pointer"
                aria-label="Previous slide"
              >
                <FaChevronLeft className="text-3xl text-white opacity-25 hover:opacity-100 transition-opacity duration-300" />
              </button>
            </div>
            <div className="absolute top-1/2 right-2 -translate-y-1/2 flex items-center z-10 opacity-0 group-hover:opacity-100 transition-opacity duration-300 pointer-events-none group-hover:pointer-events-auto hidden md:flex">
              ...
              >
                <FaChevronRight className="text-3xl text-white opacity-25 hover:opacity-100 transition-opacity duration-300" />
              </button>
            </div>
          </div>
        </div>
        {/* Footer: move closer to the bottom and improve spacing */}
